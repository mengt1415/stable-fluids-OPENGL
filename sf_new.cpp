...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "MOVE MOUSE BEFORE PRESSING W", NULL, NULL);
    if (window == NULL)
    {
   ...
   // Shader pboundary("5.1.framebuffers_screen.vs", "2pboundary.fs");
    ...
    Shader advect("5.1.framebuffers_screen.vs", "4advect.fs");
    ...
    Shader advectcolor("5.1.framebuffers_screen.vs", "4advectcolor.fs");
    Shader cboundary("5.1.framebuffers_screen.vs", "3cboundary.fs");
    Shader vort("5.1.framebuffers_screen.vs", "4vort.fs");
    ...
    //scene 1
    Shader force("5.1.framebuffers_screen.vs", "4force.fs");
    force.use();
    force.setInt("uv1", 1);
    Shader zeroforce("5.1.framebuffers_screen.vs", "4zeroforce.fs");
    zeroforce.use();
    zeroforce.setInt("uv1", 1);
    Shader adddye("5.1.framebuffers_screen.vs", "4forcedye.fs");//acting like force
    adddye.use();
    adddye.setInt("uv1", 1);
   ...
   // glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, borderColor);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (clicked == true)
        {
            ...
            force.setFloat("lx", lastX);
            force.setFloat("ly", lastY);
            force.setFloat("ox", oldX);
            force.setFloat("oy", oldY);
            ...
            glBindFramebuffer(GL_FRAMEBUFFER, dyefb1); 
            float r = sin(currentFrame) * 0.5 + 0.5;
            float g = sin(2*currentFrame) * 0.5 + 0.5;
            float b = sin(3*currentFrame) * 0.5 + 0.5;
            adddye.use();
            adddye.setFloat("lastX", lastX);
            adddye.setFloat("lastY", lastY);
            adddye.setFloat("r", r);
            adddye.setFloat("g", g);
            adddye.setFloat("b", b);
            glActiveTexture(GL_TEXTURE1);
        ...
        {
            glBindFramebuffer(GL_FRAMEBUFFER, uvfb1);
            zeroforce.use(); // set p0 for the begin of pressure jacob iteration
            //force.setFloat("lx", lastX);
           // force.setFloat("ly", lastY);
            //force.setFloat("dx", dirx);
            //force.setFloat("dy", diry);
            //cout << lastX << " " << lastY << endl;
            //cout << dirx << " " << diry << endl;
            glActiveTexture(GL_TEXTURE1);
            glBindTexture(GL_TEXTURE_2D, uv3);
        ...
        for (int i = 0; i < 150; i++) // 130 or 230
        {
            glBindFramebuffer(GL_FRAMEBUFFER, uvfb3);
...
void mouse_callback(GLFWwindow* window, double xposIn, double yposIn)
{
    ...
    if (xpos < 0.0)
    {
        xpos = ((int)xpos % (int)800) + 800;
    }
    if (xpos > 800)
    {
        xpos = (int)xpos % (int)800;
    }
    if (ypos < 0.0)
        ypos = ((int)ypos % (int)800) + 800;
    if (ypos > 800)
        ypos = (int)xpos % (int)800;
    if (firstMouse)
    {
        lastX = xpos;
        lastY = ypos;
    ...
    lastX = xpos;
    lastY = ypos;
    //cout << lastX << " " << lastY << " " << oldX << " " << oldY<< " " << lastX-oldX << " " << lastY-oldY << endl;
    double nx, ny;
